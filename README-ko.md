...
> 정규 표현식은 텍스트에서 특정 패턴을 찾는데 사용되는 문자나 심볼릭의 집합입니다.

정규 표현식(Regular expression)은 문자열에 대해 왼쪽에서 오른쪽으로 일치하는 패턴으로, 보통 "regex" 또는 "regexp", "정규식"으로 줄여 말할 수 있습니다. 정규 표현식은 문자열 재배치, validating form, 패턴 일치에 따라 문자열에서 하위 문자열 추출 등에 사용됩니다.

사용자가 username을 입력할 때 규칙(username에 문자, 숫자 밑줄(`_`), 하이픈(`-`)만 포함하고 길이 제한)을 설정한다고 가정할 때 다음과 같은 정규 표현식을 사용할 수 있습니다:
<br/><br/>
<p align="center">
<img src="https://i.imgur.com/ekFpQUg.png" alt="Regular expression">
</p>

위의 정규식은 `join_doe`, `jo-hn_doe`, `john12_as`를 허용합니다. 그러나 대문자와 너무 짧은 문자열은 허용하지 않으므로 `Jo`는 매칭되지 않습니다.
